    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <div className="mt-2 text-2xl font-semibold text-slate-900 dark:text-slate-100">{activeProjects.length}</div>
        ...
        <Card>
          ...
          <div className="mt-2 text-2xl font-semibold text-slate-900 dark:text-slate-100">{currency(openInvoiceSum)}</div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <ul className="space-y-3">
            {followUps.map((p) => (
              <li key={p.id} className="rounded-lg border border-slate-100 px-3 py-2.5 dark:border-slate-800">
          ...
          {employees.map((e) => {
            ...
            return (
              <div key={e.id} className="flex items-center gap-4">
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-semibold text-white ${e.color}`}>
                  {e.initials}
                </div>
                <div className="w-36 shrink-0 text-sm text-slate-700 dark:text-slate-300">{e.name}</div>
      ...
      <Card>
        ...
        <ul className="space-y-3">
          {openTasks.map((t) => {
            ...
            return (
              <li key={t.id} className="rounded-lg border border-slate-100 px-3 py-3 dark:border-slate-800">
                <div className="flex items-center justify-between">
  ...
  return (
    <div>
      ...
      <p className="mb-6 mt-1 text-sm text-slate-500">
        {role === 'geschaeftsfuehrung' ? 'Gesamtüberblick über alle Projekte, Kosten und offenen Punkte.' : 'Deine Aufgaben und dein Arbeitstag.'}
      </p>
