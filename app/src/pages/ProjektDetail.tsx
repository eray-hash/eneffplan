    ...
    <div>
      ...
      <div className="mt-2 flex items-start justify-between">
        ...
        <div className="text-right">
          ...
          <div className="text-lg font-semibold text-slate-800">{currency(project.offerAmount)}</div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <Card>
            <CardHeader
              title="Meilensteine"
              subtitle="Bei Abschluss aller Aufgaben eines Meilensteins wird die Teilrechnung automatisch vorgeschlagen"
            />
            <ul className="space-y-3">
              {projectMilestones.map((m) => {
                ...
                return (
                  <li key={m.id} className="rounded-lg border border-slate-100 p-3.5">
                    ...
                    <div className="mt-1 text-xs text-slate-400">
                      {m.invoicePercent}% der Angebotssumme = {currency((project.offerAmount * m.invoicePercent) / 100)}
                    </div>
                      ...
                      {milestoneTasks.map((t) => {
                        ...
                        return (
                          <li key={t.id} className="flex items-center gap-3 text-sm">
                            <div
                              className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-semibold text-white ${assignee.color}`}
                            >
                              {assignee.initials}
                            </div>
                ...
                {projectInvoices.map((inv) => (
                  <li key={inv.id} className="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 text-sm">
                    <span className="text-slate-600">
                      {currency(inv.amount)} · fällig {inv.dueOn}
                    </span>
                    <Badge tone={inv.status === 'bezahlt' ? 'green' : inv.status.startsWith('mahnung') ? 'red' : 'blue'}>
                      {inv.status === 'bezahlt'
                        ...
                        : inv.status === 'versendet'
                          ? 'Versendet'
                          : `Mahnstufe ${inv.status.split('_')[1]}`}
                    </Badge>
                  </li>
                ))}
              </ul>
            ...
            <Card>
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium text-slate-800">{currency(project.offerAmount)}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-slate-500">Interne Kosten (bisher)</dt>
                  <dd className="font-medium text-slate-800">{currency(margin.cost)}</dd>
                </div>
